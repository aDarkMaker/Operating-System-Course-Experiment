...
- 完善PKE操作系统的栈缺页异常处理
- 使得执行obj/app_sum_sequence时获得如下结果
...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval) {
  ...
  switch (mcause) {
    case CAUSE_STORE_PAGE_FAULT:
    case CAUSE_LOAD_PAGE_FAULT: {
      uint64 fault_addr = ROUNDDOWN(stval, PGSIZE);
      void *page = alloc_page();
      if (page == 0) panic("handle_user_page_fault: no mem");
      memset(page, 0, PGSIZE);
      uint64 pa = (uint64)page;
      int perm = prot_to_type(PROT_READ | PROT_WRITE, 1);
      user_vm_map(current->pagetable, fault_addr, PGSIZE, pa, perm);
      break;
    }
    default:
      sprint("unknown page fault.\n");
      panic("handle_user_page_fault: unexpected cause");
  }
}
