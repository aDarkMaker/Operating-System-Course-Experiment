...
uint64 sys_user_allocate_page() {
  ...
  if (current->user_heap.free_pages_count > 0) {
    ...
    if (!found) {
      int idx = current->total_mapped_region++;
      current->mapped_info[idx].va = USER_FREE_ADDRESS_START;
      current->mapped_info[idx].npages = 1;
      current->mapped_info[idx].seg_type = HEAP_SEGMENT;
    }
  }
  user_vm_map((pagetable_t)current->pagetable, va, PGSIZE, (uint64)pa,
         prot_to_type(PROT_WRITE | PROT_READ, 1));
...
ssize_t sys_user_wait(int pid) {
  while (1) {
    int child_found = 0;
    for (int i = 0; i < NPROC; i++) {
      if (procs[i].parent == current && (pid == -1 || procs[i].pid == pid)) {
