...
- 完善PKE操作系统内核的循环轮转调度支持
- 使得执行obj/app_two_long_loops时产生如下输出
...
int free_process( process* proc ) {
  ...
  for (int i = 0; i < proc->total_mapped_region; i++) {
    ...
    if (region->seg_type == CODE_SEGMENT) {
      continue;
    }
    if (region->seg_type == SYSTEM_SEGMENT) {
      continue;
    }
    if (region->seg_type == CONTEXT_SEGMENT) {
      continue;
    }
    user_vm_unmap(proc->pagetable, region->va, region->npages * PGSIZE, 1);
  }
