...
- 修改PKE内核
- 使得user/app0.c、user/app1.c的代码执行获得如下输出
...
hartid = 1: >>> app1 is executed by hart1
hartid = 0: >>> app0 is executed by hart0
...
#define USER_BASE_HART(h) (0x81000000UL + (unsigned long)(h) * 0x400000UL)
#define USER_STACK(h) (USER_BASE_HART(h) + 0x100000UL)
#define USER_KSTACK(h) (USER_BASE_HART(h) + 0x200000UL)
#define USER_TRAP_FRAME(h) (USER_BASE_HART(h) + 0x300000UL)

#endif
```

- 在`kernel/elf.h`中，将声明 `void load_bincode_from_host_elf(process *p);` 改为：
```c
...
extern process *current_per_hart[NCPU];
#define current (current_per_hart[read_tp()])

#endif
```

- 用以下代码替换`kernel/process.c`
```c
#include "riscv.h"
#include "strap.h"
#include "config.h"
...
process *current_per_hart[NCPU];

void switch_to(process* proc) {
  assert(proc);
  uint64 hartid = read_tp();
  current_per_hart[hartid] = proc;
...
void load_bincode_from_host_elf(process *p, uint64 hartid) {
  ...
  long r = frontend_syscall(HTIFSYS_getmainvars, (uint64)&arg_bug_msg,
      sizeof(arg_buf), 0, 0, 0, 0, 0);
  kassert(r == 0);

  size_t pk_argc = arg_bug_msg.buf[0];
  uint64 *pk_argv = &arg_bug_msg.buf[1];

  size_t app_index = 1 + hartid;
  if (app_index >= pk_argc) panic("No application for this hart.\n");

  char *app_path = (char *)(uintptr_t)pk_argv[app_index];
...
ssize_t sys_user_print(const char* buf, size_t n) {
  if (n > 0 && buf[n - 1] == '\n')
    sprint("hartid = %d: %s", (int)read_tp(), buf);
  else
    sprint("hartid = %d: %s\n", (int)read_tp(), buf);
  return 0;
}
...
ssize_t sys_user_exit(uint64 code) {
  ...
  int old;
  asm volatile("amoadd.w %0, %2, (%1)" : "=r"(old) : "r"(&exit_count), "r"(1) : "memory");
  if (old + 1 == NCPU) {
...
static volatile int boot_counter;
...
void m_start(uintptr_t hartid, uintptr_t dtb) {
  ...
  sync_barrier((int *)&boot_counter, NCPU);
